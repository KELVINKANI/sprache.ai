...
export default function HomePage() {
    ...
    if (prompt.length > 0) {
      try {
        ...
        if (response.status === 200) {
          const speechFile = await response.json();
  ...
  return (
    <main className="font-primary relative flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-gray-900 via-slate-900 to-zinc-900 text-white">
...
function ChatSection({ query, text, setText }: any) {
  ...
  return (
    <div className="w-[80  flex h-full flex-row  items-center justify-center space-x-2 rounded-2xl bg-zinc-700 p-4 md:w-[50%]">
      <Textarea
